...
5. Final Project (Web and MCP): Here, I combined the loop from build2 and the TUI from build3 into my final agent.py file. I also added live real-world tools. I used the Serper API for "search_web" and trafilatura for "read_page". For the AlphaXiv MCP server requirement, I built custom python tools (discover_papers and get_paper_content) to directly search the ArXiv scientific database. Now the bot can actually read and summarize real academic papers.
