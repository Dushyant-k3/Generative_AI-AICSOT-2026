...
def write_file(path: str, content: str) -> dict:
    ...
    try:
        with open(path,'w') as handle:
            ...
            s = handle.write(content)
            return {"success": True, "path": path , "bytes_written": s }
    except Exception as e:
...
def strip_tool_call(response_text: str) -> str:
    ...
    """
    text = re.sub(r"<tool_call>(.*?)</tool_call>","",response_text, flags= re.DOTALL)
...
def dispatch(name: str, arguments: dict) -> str:
    ...
    try:
        ...
        resulting_dict = tool_function(**arguments)
...
def run_agent(user_message: str) -> str:
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        if tool_call:
            print(f"Agent is calling tool: {tool_call['name']}")
            
            result_json = dispatch(tool_call["name"],tool_call["arguments"])
            messages.append({
                "role": "user",
