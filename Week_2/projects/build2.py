...
def dispatch(tool_call) -> str:
    ...
    try:
        ...
        return json.dumps({"error": "Failed to arguments."})
    
    if name not in TOOL_REGISTRY:
        return json.dumps({"error":f" No tool : {name} found."})
    
    try:
        tool_function = TOOL_REGISTRY[name]
        result_dict = tool_function(**arguments)
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        if finish_reason == "tool_calls":
            ...
            for tool_call in message.tool_calls: 
                print(f"Agent is call {tool_call.function.name}")
